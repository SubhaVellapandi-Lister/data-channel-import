...
export class Translate extends BaseProcessor {
   ...
   */
  public async before_translate(input: IStepBeforeInput) {
    ...
    if (
      !configParamters.translateConfig ||
      configParamters.translateConfig === {}
    ) {
    ...
    if (!configParamters.inputFileNames) {
      throw new Error(
        "Kindly specify the 'inputFileNames' parameter in the config."
      );
    }
  ...
  public async translate(
    ...
    this.fileToTranslate =
      this.inputFileNames.find((fileName: string) =>
        inputName.toLowerCase().includes(fileName)
      ) || "";
    ...
    if (this.fileToTranslate === "") {
      throw new Error(
        `Failed to validate translate processor missing input file named ${inputName}.`
      );
    }
    this.currentRow = raw;
    ...
    if (index === 1) {
      this.config = _.cloneDeep(
        input.parameters!["translateConfig"]
      ) as ITranslateParameters;
      if (this.config === null) {
        throw new Error("Missing translateConfig in Translate-Builtin");
      }
    ...
    const fileValueMappingConfig = this.config[this.fileToTranslate]
      .valueMappings;
    const newRow: string[] = [];
  ...
  private mappedHeader(original: string, index: number): string {
    if (
      (this.config[this.fileToTranslate].headerMappings ||
        this.config[this.fileToTranslate].indexMappings) &&
      // eslint-disable-next-line no-undefined
    ...
    if (this.config[this.fileToTranslate].headerMappings) {
      return (
        this.config[this.fileToTranslate].headerMappings[original] ?? original
      );
  ...
  private async checkHeaderRow(inputFileName: string): Promise<void> {
    ...
    if (
      this.originalHeaders.length !==
      Object.values(this.config[this.fileToTranslate].indexMappings).length
    ) {
      throw new Error(
        "Headerless column length does not match mapping. Be sure indexMapping accounts for all columns"
      );
    }
    const sortedCols = _.sortBy(
      Object.entries(this.config[this.fileToTranslate].indexMappings),
      (val) => Number(val[0])
    ).map((keyVal) => keyVal[1]);
  ...
  private async saveIndexMappings(): Promise<void> {
    ...
    if (this.config[this.fileToTranslate].headerMappings === null) {
      ...
      this.config[this.fileToTranslate].headerMappings = _.fromPairs(
        _.zip(this.originalHeaders, this.newHeaders)
      );
    }
