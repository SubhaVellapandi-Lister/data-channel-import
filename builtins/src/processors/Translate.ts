...
export class Translate extends BaseProcessor {
  ...
  public async translate(input: IRowProcessorInput): Promise<IRowProcessorOutput> {
      ...
      if (input.index === 1) {
          ...
          if (this.config.removeEmptyHeaders === undefined || this.config.removeEmptyHeaders === true || this.config.removeUnmappedHeaders === true) {
          ...
          if (this.config.headerlessFile) {
              ...
              return {
                  index: input.index,
                  outputs: { [`${input.name}Translated`]: this.currentRow }
              }
          }

          return {
              index: input.index,
              outputs: { [`${input.name}Translated`]: this.newHeaders }
          }
      }
      ...
      return {
          index: input.index,
          outputs: { [`${input.name}Translated`]: newRow }
      }
  }
  ...
  private async checkHeaderRow(name: string): Promise<void> {
      ...
      // @ts-ignore
      this._outputStreams.writeOutputValues({ [`${name}Translated`]: sortedCols });
  }
